...
void LinkedList<T>::insertOrdered(const T& newData) {
  ...
  //if node is smallest
  else if (fresh->data >= newnode->data) { 
    pushFront(newData);
  }
  
  

  //if node is somewhere through list
  else{
    Node *thru = head_;
...
LinkedList<T> LinkedList<T>::merge(const LinkedList<T>& other) const {
  ...
  //if (left.front() == nullptr){
  //  while (right.front() != nullptr) {
  //    tail_->next = right.front()
  //  }

  //  if (left.front()->data <= right.front()->data) {
  //    tail_->next = left.front();
  //    left.popFront();
  //  }
