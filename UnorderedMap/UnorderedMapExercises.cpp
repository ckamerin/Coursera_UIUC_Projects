...
StringIntMap makeWordCounts(const StringVec& words) {
  ...
  StringVec unique_words;

//  for(auto i = 0; i < words.size(); i++){
//    if(unique_words.find(words[i])){
//     unique_words.push_back(words[i]);
//  }
//  }
//  for(auto i = 0; i < unique_words.size(); i++){
//    int count = words.count(unique_words[i]);
//    wordcount_map[unique_words[i]] = count;
//  }  

  for(auto i = 0; i < words.size(); i++){
    int uniquesum = 0;
    for(auto j =0; j < unique_words.size(); j++){
...
//  std::cout << "word count" << countsum << words.size() << std::endl;
    
    //if(std::find(wordcount_map.begin(), wordcount_map.end(), words[i]) == wordcount_map.end()){
    //int count = std::count(words.begin(), words.end(), words[i]);
    //StringIntPair p(words[i],count);  
    //wordcount_map.insert(p); 
  //}
  ...
  //      count = count + 1;
  //    }
   //   if(wordcount_map.find(words[i])->second > 0){
  //      wordcount_map[words[i]]=count;
  //    } 
  //  }
  //}
  // =================================================
...
if (wordcount_map.find(key) != wordcount_map.end()){
  auto search = wordcount_map.find(key);
  val = search->second;
}
