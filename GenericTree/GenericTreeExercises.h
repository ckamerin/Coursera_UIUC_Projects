...
static void treeFactory(GenericTree<int>& tree) {
    ...
    rootnode->data = 4;
    auto nodeeight = rootnode->addChild(8);
    auto nodefifteen = rootnode->addChild(15);
    auto nodesixteen = nodeeight->addChild(16);
    auto nodetwentythree = nodeeight->addChild(23);
    auto nodefourtytwo = nodesixteen->addChild(42);
...
std::vector<T> traverseLevels(GenericTree<T>& tree) {
  ...
  levels.push(rootNodePtr);
  results.push_back(rootNodePtr->data);

  while (!levels.empty()){
      auto tmp = levels.front();
      levels.pop();
      int itsize = tmp->childrenPtrs.size();
      for(int i = 0; i < itsize; ++i){
        levels.push(tmp->childrenPtrs[i]);
        results.push_back(tmp->childrenPtrs[i]->data); 
    }
  }
